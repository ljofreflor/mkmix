...
constrained.mixed.model.mcmc <- function(n.iter, data){
  ...
  m <- ncol(U)/2
  n <- length(Y)
  data_sim <- matrix(ncol = 10, nrow = n.iter)
  beta.g <- data$beta
  eta.g <- data$eta
  ...
  chain <- matrix(nrow = n.iter, ncol = 1000)
  ...
  for (t in 1:n.iter) {
    ...
    chain[t, (length(beta.g) + length(eta.g) + 2):(length(beta.g) + length(eta.g) + 3)] <- tau00_2.g
    ...
    chain[t, (length(beta.g) + length(eta.g) + 3):(length(beta.g) + length(eta.g) + 4)] <- tau11_2.g
  }
